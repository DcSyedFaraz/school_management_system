    ...
    public function teachers()
    {
        if (Session::get('adminLoggedin') == true) {
            $userData = User::leftJoin('regions', 'regions.regionId', '=', 'users.regionId')
                ...
                ->select('users.userId', 'users.user_name', 'users.userName', 'users.email', 'users.mobile', 'users.registrationNumber', 'users.isActive', 'regions.regionName', 'districts.districtName', 'wards.wardName', 'schools.schoolName')->where([
                        ['users.isDeleted', '=', '0'],
                        ['users.userType', '=', 'T']
        ...
        if (Session::get('adminLoggedin') == true) {
            ...
            $req->validate(
                [
                    ...
                    'user_name' => 'required|regex:/^[^\s]+$/|unique:users,user_name',
                    'otp' => 'required|regex:/^[^\s]+$/',
    ...
    public function adminInfo($id)
    {
        if (Session::get('adminLoggedin') == true) {
            ...
            if ($adminData) {
                ...
                $data['user_name'] = $adminData['user_name'];
    ...
    public function updateAdmin(Request $req)
    {
        if (Session::get('adminLoggedin') == true) {
            ...
            if ($validUser) {
                ...
                $validUser['user_name'] = $req['user_name'];
