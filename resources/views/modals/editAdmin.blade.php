<!-- New Area Modal -->
<div id="editAdminModal" tabindex="-1" aria-hidden="true"
    class="fixed top-0 left-0 bg-black/25 border border-black right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[100vh] max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button"
                    ...
                    data-modal-hide="editAdminModal">
                    ...
                    <form action="{{ $url2 }}" method="post" id="adminForm2">
                        ...
                        <div class="grid lg:grid-cols-3 md:grid-cols-3 grid-cols-1 gap-x-5">
                            <div class="my-3">
                                ...
                                <input type="text" class="p-2 rounded-md w-full border border-black"
                                    name="updatedUsername" id="username2" placeholder="Enter Username"
                                    value="{{ old('updatedUsername') }}" required>
                            ...
                            <div class="my-3">
                                ...
                                <input type="email" class="p-2 rounded-md w-full border border-black"
                                    name="updatedEmail" id="email2" placeholder="Enter Email Id"
                                    value="{{ old('updatedEmail') }}" required>
                                @error('updatedEmail')
                                    <span class="text-red-500 capitalize text-sm italic">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="my-3">
                                ...
                                <input type="number" min="1" class="p-2 rounded-md w-full border border-black"
                                    name="updatedContactNumber" id="contactNumber2" placeholder="Enter Contact Number"
                                    value="{{ old('updatedContactNumber') }}" required>
                            ...
                            <div class="my-3">
                                ...
                                <input type="text" min="1" class="p-2 rounded-md w-full border border-black"
                                    name="user_name" id="user_name2" placeholder="Enter User Name"
                            ...
                            <div class="my-3">
                                <label for="2" class="block">One Time Password:</label>
                                <input type="text" min="1" class="p-2 rounded-md w-full border border-black"
                                    name="otp" id="otp2" placeholder="Enter One Time Password"
                                    value="{{ old('otp') }}" >
                                @error('otp')
                                ...
                                <select class="p-2 rounded-md w-full border border-black" name="updatedRegion"
                                    ...
                                    @if (count($regions) > 0)
                                        @foreach ($regions as $region)
                                            <option value="{{ $region['regionId'] }}" @selected(old('updatedRegion') == $region['regionId'])>
                                    ...
                                    @if (count($districts) > 0)
                                        @foreach ($districts as $district)
                                            <option value="{{ $district['districtId'] }}"
                                                @selected(old('updatedDistrict') == $district['districtId'])>
                                ...
                                <select class="p-2 rounded-md w-full border border-black" name="updatedWard"
                                    ...
                                    @if (count($wards) > 0)
                                        @foreach ($wards as $ward)
                                            <option value="{{ $ward['wardId'] }}" @selected(old('updatedWard') == $ward['wardId'])>
                                        ...
                                        @if (count($schools) > 0)
                                            @foreach ($schools as $school)
                                                <option value="{{ $school['schoolId'] }}"
                                                    @selected(old('updatedSchool') == $school['schoolId'])>{{ $school['schoolName'] }}</option>
                                            @endforeach
                                        @else
                                            <option value="" class="text-red-500">No Data Found!</option>
                                        @endif
                        ...
                        @if (Session::get('pageTitle') == 'Admins')
                            <div class="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-x-5">
                                <div class="my-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" class="p-2 rounded-md w-full border border-black"
                                            name="updatedPassword" id="updatedPassword" placeholder="Enter Password">
                                        <i class="material-symbols-outlined absolute right-2 top-2.5 cursor-pointer"
                                            id="uEye"
                                            onclick="showText('updatedPassword', 'uEye')">visibility</i>
                                    </div>
                                ...
                                <div class="my-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" class="p-2 rounded-md w-full border border-black"
                                            name="updatedConfirmPassword" id="updatedConfirmPassword"
                                            placeholder="Confirm Password">
                                        <i class="material-symbols-outlined absolute right-2 top-2.5 cursor-pointer"
                                            id="ufEye"
                                            onclick="showText('updatedConfirmPassword', 'ufEye')">visibility</i>
                                    </div>
            ...
            <div class="flex justify-end p-6 space-x-2 border-t border-gray-200 rounded-b">
                <button type="submit" form="adminForm2"
                    class="text-white bg-green-500 hover:bg-green-600 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update</button>
                <button data-modal-hide="editAdminModal" type="button"
