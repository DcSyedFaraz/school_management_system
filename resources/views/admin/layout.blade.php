            ...
            <div class="w-full bg-[#f0f0f0]">
                ...
                @if (Session::has('error'))
                    <div id="toast-error"
                        ...
                        <button type="button" id="errorDismiss"
                            class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8"
                ...
                @if (Session::has('error_long'))
                    <div id="toast-error-long"
                        class="z-20 flex items-center w-full max-w-xs p-4 mb-4 mx-auto absolute left-[50%] -translate-x-[30%] text-gray-500 bg-red-50 rounded-lg shadow"
                        role="alert">
                        <div
                            class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-orange-500 bg-orange-100 rounded-lg dark:bg-orange-700 dark:text-orange-200">
                        ...
                        <div class="ml-3 text-sm font-normal">{{ Session::get('error_long') }}</div>
                        <button type="button" id="errorLongDismiss"
                            class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8"
                            data-dismiss-target="#toast-error-long" aria-label="Close">
                            <span class="sr-only">Close</span>
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
...
<script>
    ...
    if (dismissBtn != null) {
        setTimeout(() => { dismissBtn.click(); }, 2000);
    }

    let errorDismissBtn = document.getElementById("errorDismiss");
    if (errorDismissBtn != null) {
        setTimeout(() => { errorDismissBtn.click(); }, 2000);
    }

    let errorLongDismissBtn = document.getElementById("errorLongDismiss");
    if (errorLongDismissBtn != null) {
        setTimeout(() => { errorLongDismissBtn.click(); }, 30000);
    }
